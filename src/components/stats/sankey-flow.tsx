...
function placeColumn(nodes: SankeyNode[], height: number, total: number) {
  ...
  nodes.forEach((node, index) => {
    ...
    placed.push({
      ...node,
      id: `${node.label}-${node.value}-${index.toString()}`,
      top: cursor,
      height: nodeHeight,
    })
...
export function SankeyFlow({
  ...
  const geometry = (() => {
    ...
    const leftRibbons = leftNodes.map((source, index) => {
      const slice = leftHubSlices[index]
      return {
        id: `${source.id}->${slice?.id ?? "hub"}`,
        path: buildRibbon(
          x0,
          hubLeft,
          source.top,
    ...
    const rightRibbons = rightNodes.map((target, index) => {
      const slice = rightHubSlices[index]
      return {
        id: `${slice?.id ?? "hub"}->${target.id}`,
        path: buildRibbon(
          hubRight,
          x1,
          slice.top,
    ...
    >
      {geometry && (
        <Canvas style={styles.canvas}>
          ...
          {/* Left Flow Ribbons */}
          {geometry.leftRibbons.map((ribbon) => (
            <Path
              key={ribbon.id}
              path={ribbon.path}
              color={ribbon.color}
          ...
          {/* Right Flow Ribbons */}
          {geometry.rightRibbons.map((ribbon) => (
            <Path
              key={ribbon.id}
              path={ribbon.path}
              color={ribbon.color}
